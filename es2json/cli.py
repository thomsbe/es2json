...
def run(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Query elasticsearch indices/index/documents and print them "
    ...
    parser.add_argument(
        "-use-ssl", action="store_true", help="use https instead of http"
    )
    parser.add_argument(
        "-ign-source",
        action="store_true",
    ...
    parser.add_argument(
        "-headless", action="store_true", help="don't print Elasticsearch metadata"
    )
    parser.add_argument(
        "-body",
        type=helperscripts.jsonstring_or_file,
    ...
    parser.add_argument(
        "-pretty", action="store_true", help="prettyprint the json output"
    )
    parser.add_argument(
        "-verbose", action="store_true", help="print progress for large dumps"
    )
    parser.add_argument(
        "-chunksize",
        type=int,
    ...
    es_kwargs["port"] = int(
        args.server.split(":")[2].rsplit("/")[0]
    )  # raise Error if port not castable to int
    if args.use_ssl:
    ...
    if args.size:
        ...
        if isinstance(
            args.size, int
        ):  # oh, we got an single number, not a string with an number or even an string describing a slice
            args.size = str(args.size)
        if ":" in args.size:
            es_kwargs["slice_"] = slice(
                int(args.size.split(":")[0]), int(args.size.split(":")[1]), 1
            )
        else:
            es_kwargs["slice_"] = slice(0, int(args.size), 1)

    if args.headless and args.ign_source:
        helperscripts.eprint(
            "ERROR! do not use -headless and -ign-source at the same Time!"
        )
        exit(-1)
    else:
